    ...
    removeMapAccessKey(offraidData, sessionID)
    {
        ...
        for (let item of offraidData.profile.Inventory.items)
        {
            if (item._tpl === mapKey && item.slotId !== "Hideout")
            {
                ...
                if (!helpfunc_f.getItem(mapKey)[1]._props.MaximumNumberOfUsage)
                {
                    usages = 1;
                }
                else
                ...
                if (item.upd.Key.NumberOfUsages >= helpfunc_f.getItem(mapKey)[1]._props.MaximumNumberOfUsage)
                {
...
function saveProgress(offraidData, sessionID)
{
    ...
    offraidData.profile.Inventory.items = helpfunc_f.replaceIDs(offraidData.profile, offraidData.profile.Inventory.items, offraidData.profile.Inventory.fastPanel);

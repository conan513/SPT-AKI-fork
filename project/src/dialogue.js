...
class DialogueServer
{
    ...
    generateDialogueList(sessionID)
    {
        ...
        for (let dialogueId in save_f.save_f.saveServer.profiles[sessionID].dialogues)
        {
    ...
    {
        let dialogue = save_f.save_f.saveServer.profiles[sessionID].dialogues[dialogueId];
	...
	*/
    generateDialogueView(dialogueId, sessionID)
    {
        ...
        return response_f.responseController.getBody({"messages": save_f.save_f.saveServer.profiles[sessionID].dialogues[dialogueId].messages});
    }
	...
	*/
    ...
    {
        if (save_f.save_f.saveServer.profiles[sessionID].dialogues === undefined)
        {
            this.initializeDialogue(sessionID);
        }

        let dialogueData = save_f.save_f.saveServer.profiles[sessionID].dialogues;
	...
	*/
    getMessageItemContents(messageId, sessionID)
    {
        ...
        for (let dialogueId in dialogueData)
        {
            ...
            for (let message of messages)
            {
                ...
                {
                    let attachmentsNew = save_f.save_f.saveServer.profiles[sessionID].dialogues[dialogueId].attachmentsNew;
                    if (attachmentsNew > 0)
                    {
                        save_f.save_f.saveServer.profiles[sessionID].dialogues[dialogueId].attachmentsNew = attachmentsNew - 1;
                    }
    ...
    {
        delete save_f.save_f.saveServer.profiles[sessionID].dialogues[dialogueId];
    }

    setDialoguePin(dialogueId, shouldPin, sessionID)
    {
        save_f.save_f.saveServer.profiles[sessionID].dialogues[dialogueId].pinned = shouldPin;
    }
    ...
    getAllAttachments(dialogueId, sessionID)
    {
        ...
        for (let message of save_f.save_f.saveServer.profiles[sessionID].dialogues[dialogueId].messages)
        {
        ...
        save_f.save_f.saveServer.profiles[sessionID].dialogues[dialogueId].attachmentsNew = 0;

...
* */
function addItem(pmcData, body, output, sessionID, callback, foundInRaid = false)
{
    ...
    for (let baseItem of body.items)
    {
        ...
        {
            const item = json.parse(json.read(database_f.database.tables.traders[fenceID].items[baseItem.item_id]));
    ...
    {
        let findSlotResult = findSlotForItem(StashFS_2D, stashX, stashY, itemToAdd.itemRef, itemLib);
...
function findSlotForItem(StashFS_2D, stashX, stashY, item, items)
{

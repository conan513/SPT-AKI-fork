...
class LocationGenerator
{
    ...
    generateContainerLoot(items)
    {
        ...
        {
            const item = Helpers.getItem(items[i]._tpl)[1];

            container2D = Helpers.fillContainerMapWithItem(
        ...
        for (let i = 0; i < minCount; i++)
        {
            ...
            while (!result.success && maxAttempts)
            {
                ...
                item = JsonUtil.clone(Helpers.getItem(rolled.id)[1]);
                ...
                result = Helpers.findSlotForItem(container2D, item._props.Width, item._props.Height);
                maxAttempts--;
            }
            ...
            if (item._props.presetId)
            {
                ...
                for (var p in preset._items)
                {
                    ...
                    {
                        let mag = Helpers.getItem(preset._items[p]._tpl)[1];
                        let cartridges = {

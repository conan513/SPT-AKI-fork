...
function locations() {
    ...
    delete file.crc;
    file.data.locations = {};
    json.write(outputDir + "locations/base.json", file);
    console.log("done: base.json");
}
...
function hideoutSettings() {
    let file = json.parse(json.read(inputDir + ".client.hideout.settings.json"));
    json.write(outputDir + "hideout/settings.json", file);
    console.log("done: settings.json");
}
        ...
        for (let target in key) {
            ...
            if (element === "items") {
                if ("upd" in key[target]) {
                    ...
                    key[target].upd = {UnlimitedCount: true, StackObjectsCount: 99999999};
                }
...
function locales() {
    ...
    localesHelper(".client.locale.sk.json", "sk");
    localesHelper(".client.locale.hu.json", "hu");
    localesHelper(".client.locale.cz.json", "cz");
    localesHelper(".client.locale.pl.json", "pl");
    localesHelper(".client.locale.jp.json", "jp");
}

function menuLocalesHelper(language, shortName) {
    let file = json.parse(json.read(inputDir + language));
    let output = outputDir + "locales/" + shortName + "/menu.json";
    json.write(output, file.data);
    console.log("done: " + output);
}

function menuLocales() {
    menuLocalesHelper("client.menu.locale.en.json", "en");
    menuLocalesHelper("client.menu.locale.ru.json", "ru");
    menuLocalesHelper("client.menu.locale.ge.json", "ge");
    menuLocalesHelper("client.menu.locale.fr.json", "fr");
    menuLocalesHelper("client.menu.locale.po.json", "po");
    menuLocalesHelper("client.menu.locale.es.json", "es");
    menuLocalesHelper("client.menu.locale.es-mx.json", "es-mx");
    menuLocalesHelper("client.menu.locale.ch.json", "ch");
    menuLocalesHelper("client.menu.locale.it.json", "it");
    menuLocalesHelper("client.menu.locale.tu.json", "tu");
    menuLocalesHelper("client.menu.locale.sk.json", "sk");
    menuLocalesHelper("client.menu.locale.hu.json", "hu");
    menuLocalesHelper("client.menu.locale.cz.json", "cz");
    menuLocalesHelper("client.menu.locale.pl.json", "pl");
    menuLocalesHelper("client.menu.locale.jp.json", "jp");
}

function globals() {
    let output = json.parse(json.read(inputDir + ".client.globals.json"));

    output.data.config.GlobalLootChanceModifier = 5;
...
function generateRagfairTrader() {
    ...
    for (let file in itemFiles) {
        ...
        // save everything
        json.write(outputDir + "assort/ragfair/items/" + fileName + ".json", {_id: fileData._id, _tpl: fileData._id, parentId: "hideout", slotId: "hideout", upd: {UnlimitedCount: true, StackObjectsCount: 99999999}});
    ...
    for (let file in globalFiles) {
        ...
        for (let item of globalFiles[file]._items) {
            if (item._id === globalFiles[file]._parent) {
                json.write(outputDir + "assort/ragfair/items/" + presetId + ".json", {_id: presetId, _tpl: item._tpl, parentId: "hideout", slotId: "hideout", upd: {UnlimitedCount: true, StackObjectsCount: 99999999}});
